...
class XrDataset(torch.utils.data.Dataset):
    ...
    def __init__(
            ...
            postpro_fn=None, test_regrid=None
    ):
        """
...
class XrDatasetRegrid(torch.utils.data.Dataset):
    ...
    def __init__(
            self, da, regrid_da, patch_dims, domain_limits=None, strides=None,
            check_full_scan=False, check_dim_order=False,
            postpro_fn=None, postpro_fn_regrid=None,test_regrid=None
    ):
        """
        da: xarray.DataArray with patch dims at the end in the dim orders
        patch_dims: dict of da dimension to size of a patch
        ...
        self.return_coords = False
        self.postpro_fn = postpro_fn
        self.postpro_fn_regrid = postpro_fn_regrid
        self.original_da = da.sel(**(domain_limits or {}))
        self.da = regrid_da.sel(**(domain_limits or {}))
        self.patch_dims = patch_dims
        self.original_patch_dims = {'time': patch_dims['time'], 'lat':test_regrid[1][0], 'lon':test_regrid[1][1]}
        self.strides = strides or {}
        self.original_strides = self.strides
        da_dims = dict(zip(self.da.dims, self.da.shape))
        self.ds_size = {
            dim: max((da_dims[dim] - patch_dims[dim]) // self.strides.get(dim, 1) + 1, 0)
            for dim in patch_dims
    ...
    def __getitem__(self, item):
        ...
        grid_item = self.da.isel(**sl)

        original_sl = {
            dim: slice(self.strides.get(dim, 1) * idx,
                       self.strides.get(dim, 1) * idx + self.original_patch_dims[dim])
            for dim, idx in zip(self.ds_size.keys(),
                                np.unravel_index(item, tuple(self.ds_size.values())))
        }
        original_item = self.original_da.isel(**original_sl)

        if self.return_coords:
            return original_item.coords.to_dataset()[list(self.original_patch_dims)]

        grid_item = grid_item.data.astype(np.float32)
        original_item = original_item.data.astype(np.float32)

        if self.postpro_fn is not None:
            grid_item = self.postpro_fn_regrid(grid_item)
            original_item = self.postpro_fn(original_item)

        return grid_item, original_item
    ...
    def reconstruct_from_items(self, items, weight=None):
        if weight is None:
            weight = np.ones(list(self.original_patch_dims.values()))
        w = xr.DataArray(weight, dims=list(self.original_patch_dims.keys()))
        ...
        da_shape = dict(zip(coords[0].dims, self.original_da.shape[-len(coords[0].dims):]))
        new_shape = dict(zip(new_dims, items[0].shape[:len(new_dims)]))

        rec_da = xr.DataArray(
            np.zeros([*new_shape.values(), *da_shape.values()]),
            dims=dims,
            coords={d: self.original_da[d] for d in self.original_patch_dims}
        )
        count_da = xr.zeros_like(rec_da)
...
class BaseDataModule(pl.LightningDataModule):
    def __init__(self, input_da, domains, xrds_kw, dl_kw, aug_kw=None, norm_stats=None, test_regrid=None, **kwargs):
        super().__init__()
    ...
    def setup(self, stage='test'):
        ...
        self.train_ds = XrDataset(
            train_data, **self.xrds_kw, postpro_fn=post_fn, test_regrid=self.test_regrid
        )
        if self.aug_kw:
            self.train_ds = AugmentedDataset(self.train_ds, **self.aug_kw)

        self.val_ds = XrDataset(
            self.input_da.sel(self.domains['val']), **self.xrds_kw, postpro_fn=post_fn, test_regrid=self.test_regrid
        )
        self.test_ds = XrDataset(
            self.input_da.sel(self.domains['test']), **self.xrds_kw, postpro_fn=post_fn, test_regrid=self.test_regrid
        )
...
class BaseDataModuleRegrid(BaseDataModule):
    def __init__(self, input_da, domains, xrds_kw, dl_kw, aug_kw=None, norm_stats=None, test_regrid=None, **kwargs):
        super().__init__(input_da, domains, xrds_kw, dl_kw, norm_stats=norm_stats, test_regrid=test_regrid, **kwargs)
        self.regridded_da = self.regrid_da(input_da)
        self.norm_stats()

    def norm_stats(self):
        if self._norm_stats is None:
            self._norm_stats = self.train_mean_std(self.regridded_da)
            print("Norm stats", self._norm_stats)
        return self._norm_stats

    def train_mean_std(self, da, variable='tgt'):
        train_data = da.sel(self.xrds_kw.get('domain_limits', {})).sel(self.domains['train'])
        return train_data.sel(variable=variable).pipe(lambda da: (da.mean().values.item(), da.std().values.item()))

    def post_fn(self, da, normalise=True):
        m = 0
        s = 1
        if normalise:
            m, s = self.train_mean_std(da)
        def normalize(item): return (item - m) / s
    ...
    def get_1_12_input(self, nadir_obs, target, new_size):
        original_mask = nadir_obs
        original_mask[~np.isnan(original_mask)] = 1 
        original_mask = np.nan_to_num(original_mask, nan=0.0)

        new_mask = cv2.resize(original_mask, new_size)
        new_mask[new_mask<0.3] = 0
        new_mask[new_mask!=0] = 1

        new_tgt = cv2.resize(target, new_size)

        new_input = new_mask * new_tgt
        new_input[new_mask==0] = np.nan

        return new_input

    def regrid_da(self, da: xr.DataArray):
        regrid_sizes = self.test_regrid[0]
        ...
        new_da = da.interp({"lat":lat_space, "lon":lon_space}, method="cubic")

        for item in da.sel({'variable':'input'}):
            new_da.loc[{'time':item.time.values, 'variable':'input'}] = self.get_1_12_input(item.values, da.sel({'time':item.time.values, 'variable':'tgt'}).to_numpy(), self.test_regrid[0])

        new_da.loc[{'variable':'tgt'}] = da.sel({'variable':'tgt'}).interp({"lat":lat_space, "lon":lon_space}, method="cubic")

        return new_da

    def setup(self, stage='test'):
        train_data = self.input_da.sel(self.domains['train'])
        train_data_regrid = self.regridded_da.sel(self.domains['train'])
        post_fn = self.post_fn(self.input_da, normalise=False)
        post_fn_regrid = self.post_fn(self.regridded_da)
        self.train_ds = XrDatasetRegrid(
            train_data, train_data_regrid, **self.xrds_kw, postpro_fn=post_fn, postpro_fn_regrid=post_fn_regrid, test_regrid=self.test_regrid
        )
        self.val_ds = XrDatasetRegrid(
            self.input_da.sel(self.domains['val']), self.regridded_da.sel(self.domains['val']), **self.xrds_kw, postpro_fn=post_fn, postpro_fn_regrid=post_fn_regrid, test_regrid=self.test_regrid
        )
        self.test_ds = XrDatasetRegrid(
            self.input_da.sel(self.domains['test']), self.regridded_da.sel(self.domains['test']), **self.xrds_kw, postpro_fn=post_fn, postpro_fn_regrid=post_fn_regrid, test_regrid=self.test_regrid
        )
